...
- MAX98357：I2S 音频测试。
- INA / INMP-class I2S Mic：当前硬件规划为 `BCLK=GPIO6`、`WS=GPIO7`、`LR=GND`、`SD=GPIO8`；当前已完成板端 I2S 采样、RMS/peak watch 和 MQTT 分片上送，Coordinator 可继续复用 `voice/stt/result` 回灌链；云端 STT 识别由本地网关/前端服务配置上游 provider 后接管。
