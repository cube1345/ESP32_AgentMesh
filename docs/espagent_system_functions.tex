...
\title{基于 ESP32/RPi 的多 Agent 智能管家系统功能说明}
\author{}
\date{}
...
\begin{abstract}
本文基于项目当前仓库实现，对多 Agent 智能管家系统的功能进行系统化梳理，并按作品说明文档格式输出为 \LaTeX{} 版本。系统以 ESP32-S3 为核心，构建了 coordinator\_agent、sensor\_agent、control\_agent、guardian\_agent 四角色协同架构，结合 ESP32-P4/网页前端展示终端与 RPi 扩展网关，形成“自然语言入口---多节点调度---环境感知---硬件执行---策略审计---结果回传”的完整闭环。系统已实现 Feishu/WebSocket/串口多入口交互、MQTT Mesh 节点协同、环境感知、执行器控制、自动化工作流、条件规则、定时任务、技能装载、长期记忆、子代理调用、可视化时间线、Sandbox 拦截展示、Guardian 策略裁决、控制急停以及语音桥接等能力。本文重点列出当前代码中已经落地的功能边界、工具能力、软件组成与扩展方向，便于后续参赛文档、答辩汇报与系统归档统一使用。
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{语音桥接}：支持结构化 TTS/STT 请求、前端语音输入、语音状态查询与浏览器回退播报。
  \item \textbf{可视化展示}：支持节点拓扑、时间线、Sandbox 事件流、Skills Studio、用户偏好面板与 AI 通信面板。
  \item \textbf{运维与扩展}：支持 OTA 计划、网关状态查看、BLE Mesh 设备注册边界、SPIFFS 文件管理和串口调试命令。
\end{enumerate}
...
展示终端 & 1 个显示/前端终端，支持网页控制台与语音前端协作 \\
注册工具能力 & 55 项已注册工具能力（以 \texttt{main/tools/tool\_registry.c} 为准） \\
Slash 指令数 & 23 项显式 Slash 命令/动作指令 \\
...
\begin{enumerate}
  ...
  \item \textbf{Coordinator 调度层}：负责 LLM 对话、工具调用、Mesh 调度、语音桥接；
  ...
  \item \textbf{展示与扩展层}：包括网页控制台、ESP32-P4 展示终端、RPi 网关与运行时 Skills。
\end{enumerate}
...
\midrule
Coordinator 节点 & 调度中枢 & 接收用户消息、构建上下文、调用工具、下发 Mesh 命令、生成回复、发起语音请求 \\
...
Display/Web 前端 & 展示终端 & 展示拓扑、时间线、Sandbox、Skills、偏好、聊天消息和浏览器语音状态 \\
RPi 网关 & 扩展网关 & 作为本地 AI/MQTT/MCP/知识库扩展载体，承接后续边缘计算能力 \\
\bottomrule
\end{tabular}
\end{table}
...
硬件系统由多块 ESP32-S3 节点和一个展示终端组成。各节点通过 MQTT Mesh 协同工作，其中 coordinator 节点承载 Agent 主循环，sensor 节点承载环境传感器，control 节点承载执行器，guardian 节点承载安全边界和审计逻辑。系统外围可进一步连接 ESP32-P4/C6 显示终端与 RPi 网关。
...
软件系统由板端 Agent Runtime、Mesh 通信层、自动化引擎、Skills/Memory 持久层、语音桥接层、前端聚合服务和网页控制台共同组成。板端负责实时感知、执行和策略判断；前端负责聚合 MQTT 数据、代理聊天网关、展示时间线和管理运行时 Skills。整体软件形成了“入口---理解---执行---审计---展示”的端到端闭环。
...
\begin{itemize}
  ...
  \item 支持 AI 通信窗口、浏览器 STT 状态显示、TTS 回退播报、运行时 Skill 管理。
...
语音桥接 & \texttt{voice\_status} & 查询 TTS/STT 语音桥状态与主题配置 \\
语音桥接 & \texttt{voice\_request\_tts} & 向显示终端发布 TTS 请求 \\
语音桥接 & \texttt{voice\_request\_stt} & 向显示终端发布 STT 请求 \\
...
网关 & \texttt{gateway\_status} & 查询网关、设备注册表和 OTA 边界状态 \\
网关 & \texttt{gateway\_register\_ble\_mesh\_device} & 注册 BLE Mesh 外部设备信息 \\
网关 & \texttt{gateway\_ble\_mesh\_send} & 发送或暂存 BLE Mesh 网关命令 \\
运维 & \texttt{ota\_gateway\_plan} & 生成 OTA 升级计划 \\
...
\begin{enumerate}
  ...
  \item 支持 55 项注册工具能力，覆盖感知、控制、自动化、技能、文件、网关、语音和运维能力。
  \item 支持 3 类自动化机制：多步工作流、条件规则、定时任务。
  \item 支持 3 类用户入口：Feishu、WebSocket 网页前端、串口 CLI。
  \item 支持 23 项 Slash 指令，实现显式路由、显式清理、显式状态查询与显式技能访问。
  ...
  \item 支持语音桥接链路，但云端 TTS/STT 的最终可用性仍依赖相应的凭据和配额。
\end{enumerate}
...
\begin{itemize}
  ...
  \item 将 OTA 从“计划生成”扩展为 Guardian 审批后的远程执行闭环；
  \item 补强板端 TTS/STT 本地资源与云配额管理，降低对浏览器回退链路的依赖；
  \item 将 BLE Mesh、RPi 本地模型、知识库和 MCP 能力进一步纳入统一调度面板；
  \item 增加更丰富的用户画像与偏好闭环，实现更主动的环境控制与消息提醒。
\end{itemize}

\subsection{心得体会}
本系统的核心经验在于：面向真实硬件的 Agent 系统，不能只追求“能回答”，更要追求“能执行、能回看、能审计、能停下”。因此在设计中引入了角色分工、Mesh 协同、自动化引擎、Guardian 审核、Sandbox 拦截和前端时间线展示等机制，使自然语言控制不再停留在单轮对话，而是能够落到实际设备、后台任务和持续规则中。同时，系统保留了 Skills、Lua、设备清单和 RPi 网关等扩展边界，为后续从单一智能家居演示走向复杂边缘智能平台留下了接口。
