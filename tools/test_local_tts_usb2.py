...
def main() -> int:
    ...
    try:
        ...
        result = drain(fd, 60.0, echo)
        ok = (
            ("local_tts_demo_zh status: ESP_OK" in result or "local_tts_b64 status: ESP_OK" in result or "local_tts_speak status: ESP_OK" in result)
