...
"<div class='card collapsed' id='sec-status'>"
"<div class='card-hdr' onclick='toggle(this)'>Device Status</div>"
"<div class='card-body'>"
"<button class='btn btn-scan' onclick='loadStatus()'>Refresh Status</button>"
"<pre id='device_status' style='white-space:pre-wrap;font-size:.78em;background:#f7f7f7;border:1px solid #ddd;border-radius:8px;padding:10px;max-height:220px;overflow:auto'></pre>"
"</div></div>"
...
"loadConfig();"
"function loadStatus(){"
"fetch('/status').then(r=>r.json()).then(j=>{document.getElementById('device_status').textContent=JSON.stringify(j,null,2)})"
".catch(e=>{document.getElementById('device_status').textContent='status error: '+e})}"
"</script>"
"</body></html>";
