...
/* Gateway section */
"<div class='card collapsed' id='sec-gateway'>"
"<div class='card-hdr' onclick='toggle(this)'>Gateway Status</div>"
"<div class='card-body'>"
"<button class='btn btn-scan' onclick='loadGateway()'>Refresh Gateway</button>"
"<pre id='gateway_status' style='white-space:pre-wrap;font-size:.78em;background:#f7f7f7;border:1px solid #ddd;border-radius:8px;padding:10px;max-height:220px;overflow:auto'></pre>"
"</div></div>"

"<div class='card collapsed' id='sec-ble'>"
"<div class='card-hdr' onclick='toggle(this)'>BLE Mesh Device</div>"
"<div class='card-body'>"
"<label>Device ID</label>"
"<input id='ble_device_id' placeholder='blemesh_living_room_light'>"
"<label>Address</label>"
"<input id='ble_address' placeholder='0005'>"
"<label>Name</label>"
"<input id='ble_name' placeholder='Living room light'>"
"<label>Capabilities</label>"
"<input id='ble_capabilities' placeholder='light,onoff,rgb'>"
"<button class='btn btn-scan' onclick='registerBle()'>Register BLE Mesh Device</button>"
"<pre id='ble_result' style='white-space:pre-wrap;font-size:.78em'></pre>"
"</div></div>"

"<div class='card collapsed' id='sec-ota'>"
"<div class='card-hdr' onclick='toggle(this)'>OTA Gateway Plan</div>"
"<div class='card-body'>"
"<label>Target Node</label>"
"<input id='ota_target_node' placeholder='esp32s3-control-01'>"
"<label>Target Role</label>"
"<input id='ota_target_role' placeholder='control_agent'>"
"<label>Firmware URL</label>"
"<input id='ota_url' placeholder='https://example.com/ESPAgent.bin'>"
"<label>Version</label>"
"<input id='ota_version' placeholder='v1.2.3'>"
"<button class='btn btn-scan' onclick='planOta()'>Create OTA Plan</button>"
"<pre id='ota_result' style='white-space:pre-wrap;font-size:.78em'></pre>"
"</div></div>"
...
"fetch('/status').then(r=>r.json()).then(j=>{document.getElementById('gateway_status').textContent=JSON.stringify(j,null,2)})"
".catch(e=>{document.getElementById('gateway_status').textContent='status error: '+e})}"
"function registerBle(){"
"var data={device_id:document.getElementById('ble_device_id').value.trim(),"
"address:document.getElementById('ble_address').value.trim(),"
"name:document.getElementById('ble_name').value.trim(),"
"capabilities:document.getElementById('ble_capabilities').value.trim()};"
"fetch('/gateway/ble_mesh/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)})"
".then(r=>r.json()).then(j=>{document.getElementById('ble_result').textContent=JSON.stringify(j,null,2);loadGateway()})"
".catch(e=>{document.getElementById('ble_result').textContent='register error: '+e})}"
"function planOta(){"
"var data={target_node:document.getElementById('ota_target_node').value.trim(),"
"target_role:document.getElementById('ota_target_role').value.trim(),"
"url:document.getElementById('ota_url').value.trim(),"
"version:document.getElementById('ota_version').value.trim(),confirmed:false};"
"fetch('/ota/plan',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)})"
".then(r=>r.json()).then(j=>{document.getElementById('ota_result').textContent=JSON.stringify(j,null,2)})"
".catch(e=>{document.getElementById('ota_result').textContent='ota plan error: '+e})}"
"</script>"
"</body></html>";
