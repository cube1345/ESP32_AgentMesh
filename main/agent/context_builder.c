...
#define CONTEXT_SECTION_MAX_LONG  2048
#define CONTEXT_SECTION_MAX_MED   1536
#define CONTEXT_SECTION_MAX_SHORT 1024
...
static void truncate_plain_text(const char *src, char *dst, size_t dst_size, size_t max_chars)
{
    if (!dst || dst_size == 0) {
        return;
    }
    ...
    memcpy(dst, src, keep);
    dst[keep] = '\0';
    terminate_at_utf8_boundary(dst, keep);
    ...
    if (src_len > strlen(dst) && strlen(dst) + 16 < dst_size) {
        strncat(dst, "\n[truncated]", dst_size - strlen(dst) - 1);
    }
}

static size_t append_section_text_limited(char *buf,
                                          size_t size,
                                          size_t offset,
                                          const char *header,
                                          const char *text,
...
{
    char limited[4096];
    truncate_plain_text(text, limited, sizeof(limited), max_chars);
    if (limited[0] == '\0') {
        return offset;
    }
...
esp_err_t context_build_system_prompt(char *buf, size_t size)
{
    ...
    if (coordinator_compact) {
        off = append_format(
            ...
            "- Treat slash commands such as /sensor /control /guardian /mesh /status /trace as strong routing constraints.\n"
            "- Single-tool mode is active: execute at most one tool function for each user request. Never chain a second tool call in the same turn.\n"
            "- Multi-step workflows and cron schedules are disabled. Runtime Skill @condition rules are allowed only as one sensor threshold check followed by one Guardian-reviewed Action with interval, hysteresis, and cooldown.\n"
            ...
            "- Current verified control devices are control_agent WS2812 status light, GPIO5 curtain servo, and GPIO0 IR emergency stop. Do not route ordinary requests to legacy fan/humidifier devices.\n"
            ...
            "- Never claim a Mesh command succeeded unless the tool result says it succeeded.\n"
            ...
            "- lua_runtime_info and lua_list_scripts only for explicit Lua diagnostics; do not start Lua jobs from ordinary chat.\n\n"
            "## Memory And Skills\n"
            ...
            "- Runtime Skills may declare one-shot rules with: @rule trigger=\"phrase|alias\" target_role=control_agent action=set_status_light args={\"color\":\"blue\"}. A matched rule should trigger one immediate tool action only, not a workflow or background task.\n"
            "- Runtime Skills may declare one bounded condition rule with: @condition metric=humidity_percent threshold=70 hysteresis=2 interval_s=30 cooldown_s=300 above_action=set_status_light above_args={\"color\":\"red\"} below_action=set_status_light below_args={\"color\":\"off\"}. Conditions are evaluated by the coordinator automation task, and actions still require Mesh/Guardian approval.\n"
            "- When an exact Runtime Skill is injected for the current request, answer from that content directly and do not search for or combine another Skill.\n\n"
        ...
        off = append_format(
            ...
            "The user may explicitly steer execution with slash commands before natural language, such as /sensor, /control, /guardian, /local, /mesh, /status, /stop, /resume, /device, /profile, /skills, /privacy, /trace, and /help. When a slash command is present, treat it as a strong routing constraint rather than casual text.\n"
            "Single-tool mode is active. Execute at most one tool function for each user request, never chain tool calls, and ask the user to split multi-action requests. Workflow, rule, cron, and persistent background automation creation are disabled.\n\n"
            ...
            "- set_humidifier: Turn the humidifier on/off. It is wired to GPIO4 on control_agent; ON=HIGH, OFF=LOW.\n"
            "- set_fan: Turn the fan on/off. It is wired to GPIO5 on control_agent; ON=HIGH, OFF=LOW.\n"
            "- set_device_led: Turn the discrete GPIO6 LED on/off. This is not the WS2812 status light; ON=HIGH, OFF=LOW.\n"
            ...
            "Lua background jobs are not part of the current stable demo path; prefer one immediate tool action or a Runtime Skill @rule.\n\n"
            "## Agent Sandbox\n"
    ...
    off = append_format(
        ...
        "Lua is an optional script runtime. This firmware exposes lua_runtime_info, lua_list_modules, lua_list_scripts, lua_run_source, and lua_run_script for explicit diagnostics. Actual execution requires ESPAGENT_ENABLE_LUA_RUNTIME=1 and the georgik/lua component. If lua_runtime_info says unavailable, explain that scripts are supported by the API boundary but not executable in this build.\n"
        "CAN/TWAI, BLE GATT, 1-Wire, I2S/PDM, generic RMT/IR learning, USB CDC, and SDIO/SDMMC are recognized planning protocols but are not executable manifest primitives in this firmware yet. The only IR execution currently linked is the dedicated Gree AC transmit path through gree_ac_control.\n"
        ...
        "If this node is coordinator_agent and the user asks to control a remote actuator, a third board, a control board, a humidifier, a fan, a GPIO6 LED, a relay, a pump, a servo, GPIO output, a Gree air-conditioner, or a remote/status light, use mesh_send_command with target_role=\"control_agent\" and the matching action such as set_humidifier, set_fan, set_device_led, set_status_light, ws2812_set, servo_write, gpio_write, or gree_ac_control. The user does not need to know the control node id or MQTT topic. Do not execute remote actuator requests locally on the coordinator board unless the user explicitly says this board/local board.\n"
        ...
        "If this node receives a Mesh agent_task, solve it using only this role's visible tools and responsibilities, then produce a concise final reply. Sensor tasks should reason over sensing and telemetry; Control tasks should reason over actuator state and safe local controls; Guardian tasks should reason over policy, privacy, audit, and StateBoard.\n"
            "Single-tool mode is active. Execute no more than one tool function per user request. Multi-step ordered actions and cron schedules remain disabled. A Runtime Skill @condition declaration is a separately managed, bounded sensor threshold rule, not a multi-tool chat turn.\n"
        ...
        "For a Gree AC, use gree_ac_control with structured state such as power=on/off, mode=cool/heat/dry/fan/auto, temp_c or temp_delta, fan=auto/low/medium/high, and swing=on/off. Requests like '格力空调开机', '制冷26度', '制热', '温度加一度', '温度减一度', '风速调高', and '打开摆风' map to gree_ac_control.\n"
        ...
        "Examples: ask which GPIO before controlling an unspecified pin, ask for confirmation before controlling a risky external device, and ask whether to turn on or turn off when the intent is unclear.\n\n"
        "## GPIO\n"
