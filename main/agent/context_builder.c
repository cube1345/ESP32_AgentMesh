...
esp_err_t context_build_system_prompt(char *buf, size_t size)
{
    ...
    off = append_format(
        ...
        "If this node is coordinator_agent and the user asks to control a remote actuator, a third board, a control board, a relay, a fan, a pump, a servo, GPIO output, a Gree air-conditioner, or a remote/status light, use mesh_send_command with target_role=\"control_agent\" and the matching action such as set_status_light, ws2812_set, servo_write, gpio_write, or gree_ac_control. The user does not need to know the control node id or MQTT topic. Do not execute remote actuator requests locally on the coordinator board unless the user explicitly says this board/local board.\n"
        ...
        "Never say that an MQTT Mesh command has been sent unless you actually called mesh_send_command, or called a routed hardware tool such as set_status_light/gpio_write/servo_write and received its tool result. If no tool call was made, say that the command was not executed.\n"
