...
esp_err_t context_build_system_prompt(char *buf, size_t size)
{
    ...
    if (coordinator_compact) {
        off = append_format(
            ...
            "- If the user asks to list scheduled jobs, rules, workflows, or current tasks, use cron_list and/or automation_list. Do not treat list requests as requests to create a new schedule.\n"
            "- If the user says a specific scheduled job, rule, or workflow is invalid/canceled/disabled/removed or should no longer run, remove it by ID with cron_remove or automation_remove. Do not create a replacement task; ask for the exact ID if missing.\n"
        ...
        off = append_format(
            ...
            "When the user asks to list current scheduled jobs, rules, workflows, or tasks, use cron_list and/or automation_list. Do not treat list requests as requests to create a new schedule.\n"
            "When the user says a specific scheduled job, rule, or workflow is invalid, canceled, disabled, removed, or should no longer run, remove that existing task by ID with cron_remove or automation_remove. Cron IDs are shown in cron_list brackets such as 72b8f2db; automation IDs are usually rule-... or wf-.... Do not create a replacement task. If the ID is missing, ask for the exact ID or list tasks first.\n\n"
    ...
    off = append_format(
        ...
        "Gateway note: current firmware has an ESPAgent device registry, MQTT Mesh gateway, local WebSocket gateway, and a BLE Mesh bridge boundary. BLE Mesh backend is not linked unless gateway_status says enabled=true, so BLE devices can be registered and shown but cannot be truthfully claimed as physically controlled before the bridge returns OK.\n"
        "For OTA, use ota_gateway_plan for planning/visibility only. Do not claim a target node has been upgraded unless serial ota_update or a future Guardian-gated OTA execution path actually reports success.\n"
        "Voice note: current firmware now has a structured voice bridge over MQTT. Coordinator can publish TTS/STT requests, and the display front end can return STT transcript messages back into agent_loop. Without configured cloud credentials, STT/TTS may report unavailable while the MQTT/control path still works.\n"
        ...
        "Use spawn_subagent only when a subtask is meaningfully separable from the main conversation, such as researching a topic, reading a file and summarizing it, or comparing small pieces of information. Do not use spawn_subagent for hardware operations, remote Mesh control, or simple tasks that one normal tool call can finish.\n"
        ...
        "If the user asks for multiple ordered hardware actions, delays, or a sequence, use automation_create_workflow instead of blocking the agent loop or manually waiting. Example: '点亮红色，10秒后点亮蓝色' should create a workflow with two set_status_light steps and delay_ms=10000 on the second step.\n"
        "If the user asks for ongoing conditional behavior, thresholds, monitoring, or repeated linkage, use automation_create_rule instead of repeatedly chatting or relying on memory. Example: '温度大于35度亮红色，小于等于35度亮蓝色' should create a temperature_c rule with above=set_status_light red and below=set_status_light blue, with cooldown_s and hysteresis_c to avoid rapid flapping.\n"
