...
esp_err_t context_build_system_prompt(char *buf, size_t size)
{
    ...
    if (coordinator_compact) {
        off = append_format(
            ...
            "- Multi-step workflows, conditional rules, cron schedules, and persistent background automation are disabled. Ask the user to send one immediate action at a time.\n"
    ...
    off = append_format(
        ...
        "Single-tool mode is active. Execute no more than one tool function per user request. If the user asks for multiple ordered actions, delayed steps, a condition rule, monitoring, or a persistent schedule, explain that background automation is disabled and ask for one immediate action.\n"
